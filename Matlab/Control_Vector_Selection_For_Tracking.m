function [Reward] = Control_Vector_Selection_For_Tracking (k, pf,sys,obs, UAV_Sets, Ms, alpha, Area)
    Ns = pf.Ns;
...
%     gen_obs_noise =  mvnrnd(zeros(1,nv),sigma_v,1)';
    ...
    UAV_Sets_New = UAV_Sets(:,gain>=prctile(gain,80)); % Get new control action set with good gain only. 80 is the best
    nu = size(UAV_Sets,2);
    R = zeros(1,nu);                        % Reward function
    for a = 1:nu
        u = UAV_Sets(:,a);
        if ~isempty(find(sign(u(1:3) - Area(:,1))==-1)) || ~isempty(find(sign(Area(:,2) - u(1:3)) ==-1)) || sum(ismember(UAV_Sets_New', u', 'rows')) == 0
            R(a) = 0;
        else
            for m = 1:Ms
                jm = randi(Ns);
