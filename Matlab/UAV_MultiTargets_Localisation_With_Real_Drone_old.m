...
alpha = 0.5;
Area = [-1 -1 uav0(3);R_max+1 R_max+1 uav0(3)]';
...
%             Plot_Truth_UAV (truth, uav);
        ...
        while norm(prev_uav(1:3) - uav0(1:3)) > 0.5
            ...
            prev_uav = [data.position(1:2);data.position(4); data.heading /180*pi]; 
            uav(:,k) = prev_uav;
            if  mod(k,mdp_cycles(mdp_cycle))==0
                Plot_Truth_UAV (truth, uav, c);
            end
            pause(1);
        end
        Plot_Target_Estimated_Position (truth, est, uav,c);
...
export_fig SITL_Estimate_Postion.eps -transparent
%{
