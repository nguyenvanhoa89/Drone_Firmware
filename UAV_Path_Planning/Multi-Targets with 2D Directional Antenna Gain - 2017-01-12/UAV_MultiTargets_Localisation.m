...
for k = 2:T
    ...
    for i=1:ntarget 
       measurement(i) = obs(k, truth.X{i}(:,k),   gen_obs_noise(),uav(:,k));
       if isempty(est.foundTargetList(i == est.foundTargetList))
           ...
           if measurement(i) > RSS_Threshold
               ...
               if meas.ValidZCount{i}>5 && abs(std(meas.Z{i}(k-5:k))) < 2*sigma_v && abs(mean(meas.Z{i}(k-5:k)) - RSS_Threshold) < 3* sigma_v &&   norm(std(est.pf{i}.particles(:,:,k),0,2)) < sigma_v/3 %&& abs( obs(k, est.X{i}(:,k),   gen_obs_noise(),uav(:,k)) - measurement(i)) < sigma_v
