...
%         sys_noise = mvnrnd(zeros(1,nx),pf.sigma_u,pf.Ns)';
         sys_noise = pf.sigma_u*pf.sys_noise.B*randn(size(pf.sys_noise.B,2),pf.Ns);
        xk(:,:) = sys(k,pf.F, xkm1(:,:), sys_noise);
...
%         wk = diag(wkm1) * g_likelihhood';
    ...
    if rand < p_Inject
        ...
        xk(:,InjectLocation) = [pf.R_max * rand(InjectNumber,1) pf.R_max * rand(InjectNumber,1) zeros(InjectNumber,1)  (-pf.sigma_u + 2*pf.sigma_u*rand(InjectNumber,1)) (-pf.sigma_u + 2*pf.sigma_u*rand(InjectNumber,1)) ]';
    end
    %% ---resampling
