...
result = [cell2mat(est.foundIndex)';cell2mat(est.RMSFound)]
time_PF = toc;
...
huav = plot(uav(1,1:k+Cycle+1), uav(2,1:k+Cycle+1),'-', 'Color' , c(ntarget+1,:),'LineWidth',3);
hlegend = [];
count = 0;
for i=1:ntarget
...
save(['MultiTarget_Estimate',datestr(now, 'yyyymmddHHMMss'),'L5DB_5Cycles.mat'], 'truth', 'meas', 'est','uav_travel_distance_k','uav','MC_Results','mdp_cycles','c' );
% subplot(2,2,2);
% hold on;
