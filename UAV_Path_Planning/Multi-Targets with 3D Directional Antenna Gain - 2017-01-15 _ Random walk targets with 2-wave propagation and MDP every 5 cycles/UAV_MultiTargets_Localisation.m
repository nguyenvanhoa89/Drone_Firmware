...
%                measurement(i) = RSS_Threshold;
           ...
           if i == best_u && mod(k,5)==0 % Update way point every 5 seconds only
%                if mean(measurement) == RSS_Threshold
%                    % Reset particle
%                    est.pf{i}.k = k;
%                    est.pf{i}.w(:,k) =  ones(pf.Ns, T)/pf.Ns;
%                    est.pf{i}.particles(:,:,k) = est.pf{i}.gen_x0; 
%                end
                [meas.UAV{i}(:,k),meas.Reward{i}(k)]  = Control_Vector_Selection (k, est.pf{i},sys,obs, UAV_Sets, Ms, alpha, Area);
           else
               meas.Reward{i}(k) = -1; % Min value to skip
               meas.UAV{i}(:,k) = uav_model(uav(:,k),dt,vu);
%                measurement(i) = RSS_Threshold; % Min value to skip
           end
